...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save_contact'])) {
    ...
    $update_sql = "UPDATE contactTT SET hotline=?, email=? WHERE id=1";
    $stmt = $conn->prepare($update_sql);
    $stmt->bind_param("ss", $hotline, $email);
    $stmt->execute();

    echo "<script>alert('Cập nhật thông tin liên hệ thành công');</script>";
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save_content'])) {
    ...
    $sql = "UPDATE content 
            SET name=?, title=?, description1=?, description2=?, image1=?, image2=?, image3=? 
            WHERE id=1";

    $stmt = $conn->prepare($sql);
    $stmt->bind_param(
        "sssssss",
        $name, $title, $description1, $description2,
...
<?php endfor; ?>
           ...
           <button type="submit" name="save_content" class="btn btn-primary">Cập nhật</button>
