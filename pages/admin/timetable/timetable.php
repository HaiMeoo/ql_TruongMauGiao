...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_timetable'])) {
    ...
    // Đếm số dòng thời khóa biểu đã có
    $check_query = "
        SELECT COUNT(*) AS total 
        FROM timetable 
        WHERE GradeID = '$grade_id' AND date = '$date'
    ";
    $result = $conn->query($check_query);
    $row = $result->fetch_assoc();

    // Nếu đã đủ 13 tiết thì không thêm nữa
    if ($row['total'] >= 13) {
        echo "<script>alert('Thời khóa biểu đã tồn tại đầy đủ!');</script>";
    } else {

        // Thêm từng khung giờ còn thiếu
        for ($i = 1; $i <= 13; $i++) {

            // Kiểm tra khung giờ này đã tồn tại chưa
            $check_time = "
                ...
                AND TimeID = '$i'
            ";
            $time_result = $conn->query($check_time);

            // Nếu chưa tồn tại thì mới thêm
            if ($time_result->num_rows == 0) {
                $insert_query = "
                    INSERT INTO timetable (GradeID, ActivityID, TimeID, Description, date) 
                    VALUES ('$grade_id', '$i', '$i', '', '$date')
                ";
                if (!$conn->query($insert_query)) {
                    echo "Lỗi khi thêm tiết $i: " . $conn->error . "<br>";
                }
            }
        }

        // Lấy lại danh sách thời khóa biểu vừa thêm
        $filter_query = "
            SELECT t.*, g.GradeName, a.activityname, tm.timename 
            FROM timetable t 
            LEFT JOIN grade g ON t.GradeID = g.id 
            LEFT JOIN activity a ON t.ActivityID = a.id 
            LEFT JOIN time tm ON t.TimeID = tm.id 
        ...
        echo "<script>alert('Thêm / bổ sung thời khóa biểu thành công!');</script>";
    }
}
            ...
            <nav class="col-md-2 d-none d-md-block bg-dark sidebar min-vh-100" style="padding:0; position: fixed;">
                <div class="position-sticky">
                    ...
                    <ul class="nav flex-column">
                        <li class="nav-item">
                            <a class="nav-link text-white" href="/qlTruongMauGiao/pages/admin/dashboard.php">
                                <i class="fas fa-home me-2"></i>
                                Tổng quan
                            </a>
                        </li>
                        <li class="nav-item" >
                            <a class="nav-link text-white" href="/qlTruongMauGiao/pages/admin/teacher/list_teacher.php">
                                <i class="fas fa-chalkboard-teacher me-2"></i>
                                Quản lý giáo viên
                            </a>
                        </li>
                        <li class="nav-item" >
                            <a class="nav-link text-white" href="/qlTruongMauGiao/pages/admin/student/list_student.php">
                                <i class="fas fa-user-graduate me-2"></i>
                                Quản lý học sinh
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link text-white" href="/qlTruongMauGiao/pages/admin/class/list_class.php">
                                <i class="fas fa-school me-2"></i>
                                Quản lý lớp học
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link text-white" href="/qlTruongMauGiao/pages/admin/account/list_account.php">
                                <i class="fas fa-users me-2"></i>
                                Quản lý tài khoản
                            </a>
                        </li>
                        <li class="nav-item"  >
                            <a class="nav-link text-white" href="/qlTruongMauGiao/pages/admin/attendance/list_attendance.php">
                            <i class="fas fa-calendar-check me-2"></i>
                                Quản lý điểm danh
                            </a>
                        </li>
                        <li class="nav-item"style="background:#575f66">
                            <a class="nav-link text-white" href="/qlTruongMauGiao/pages/admin/timetable/timetable.php">
                                <i class="fas fa-clock me-2"></i>
                                Quản lý thời khóa biểu
                            </a>
                        </li>
                        <li class="nav-item" >
                            <a class="nav-link text-white" href="/qlTruongMauGiao/pages/admin/frontend/slide/list_slide.php">
